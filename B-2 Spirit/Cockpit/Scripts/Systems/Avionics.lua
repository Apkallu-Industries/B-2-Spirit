...
local parameters =
{
	...
	-- B-2 4-Engine Matrix & Glass Cockpit Avionics
	RPM_1                = get_param_handle("RPM_1"),
	RPM_2                = get_param_handle("RPM_2"),
	RPM_3                = get_param_handle("RPM_3"),
	RPM_4                = get_param_handle("RPM_4"),
	EGT_1                = get_param_handle("EGT_1"),
	EGT_2                = get_param_handle("EGT_2"),
	EGT_3                = get_param_handle("EGT_3"),
	EGT_4                = get_param_handle("EGT_4"),
	FF_1                 = get_param_handle("FF_1"),
	FF_2                 = get_param_handle("FF_2"),
	FF_3                 = get_param_handle("FF_3"),
	FF_4                 = get_param_handle("FF_4"),
	OIL_1                = get_param_handle("OIL_1"),
	OIL_2                = get_param_handle("OIL_2"),
	OIL_3                = get_param_handle("OIL_3"),
	OIL_4                = get_param_handle("OIL_4"),
	HEADINGCOMPASS       = get_param_handle("HEADINGCOMPASS"),
	SPEEDOMETER_IAS      = get_param_handle("SPEEDOMETER_IAS"),
	GEAR_NOSE            = get_param_handle("GEAR_NOSE"),
	GEAR_LEFT            = get_param_handle("GEAR_LEFT"),
	GEAR_RIGHT           = get_param_handle("GEAR_RIGHT"),
	ADI_ROLL_DEG         = get_param_handle("ADI_ROLL_DEG"),
	ADI_PITCH_DEG        = get_param_handle("ADI_PITCH_DEG"),
	PITCH_LADDER_Y       = get_param_handle("PITCH_LADDER_Y"),
	TOTAL_FUEL_LBS       = get_param_handle("TOTAL_FUEL_LBS"),
	EICAS_ENG_PAGE       = get_param_handle("EICAS_ENG_PAGE"),
	PFD_ADI_PAGE         = get_param_handle("PFD_ADI_PAGE"),
}
...
function update()
	...
	local rpm_l = sensor_data.getEngineLeftRPM()
	local rpm_r = sensor_data.getEngineRightRPM()
	local egt_l = sensor_data.getEngineLeftTemperatureBeforeTurbine()
	local egt_r = sensor_data.getEngineRightTemperatureBeforeTurbine()

	parameters.RPM_L        :set(rpm_l)
	parameters.RPM_R        :set(rpm_r)
	parameters.EGT_L        :set(egt_l)
	parameters.EGT_R        :set(egt_r)

	-- 4x Engine Telemetry for B-2 Spirit
	parameters.RPM_1:set(rpm_l)
	parameters.RPM_2:set(rpm_l * 0.996 + 0.15)
	parameters.RPM_3:set(rpm_r * 1.002 - 0.10)
	parameters.RPM_4:set(rpm_r)

	parameters.EGT_1:set(egt_l)
	parameters.EGT_2:set(egt_l * 0.998 + 2.0)
	parameters.EGT_3:set(egt_r * 1.003 - 1.5)
	parameters.EGT_4:set(egt_r)

	-- Fuel Flow (PPH ~ RPM * 38)
	parameters.FF_1:set(math.max(350, rpm_l * 38))
	parameters.FF_2:set(math.max(350, (rpm_l * 0.996 + 0.15) * 38))
	parameters.FF_3:set(math.max(350, (rpm_r * 1.002 - 0.10) * 38))
	parameters.FF_4:set(math.max(350, rpm_r * 38))

	-- Oil Pressure (PSI ~ 45-55 PSI nominal)
	local oil_base_l = rpm_l > 20 and (45 + rpm_l * 0.1) or (rpm_l * 1.5)
	local oil_base_r = rpm_r > 20 and (45 + rpm_r * 0.1) or (rpm_r * 1.5)
	parameters.OIL_1:set(oil_base_l)
	parameters.OIL_2:set(oil_base_l - 1.2)
	parameters.OIL_3:set(oil_base_r + 0.8)
	parameters.OIL_4:set(oil_base_r)

	-- Heading compass (radians) and pitch/roll in degrees
	parameters.HEADINGCOMPASS:set(-sensor_data.getHeading())
	local pitch_deg = sensor_data.getPitch() * 180 / math.pi
	local roll_deg  = sensor_data.getRoll() * 180 / math.pi
	parameters.ADI_PITCH_DEG:set(pitch_deg)
	parameters.ADI_ROLL_DEG:set(roll_deg)
	parameters.PITCH_LADDER_Y:set(-sensor_data.getPitch() * 1.2)

	local cur_ias = sensor_data.getIndicatedAirSpeed() * 1.944
	parameters.SPEEDOMETER_IAS:set(cur_ias)

	-- Gear down/locked status (0 = up, 1 = down)
	parameters.GEAR_NOSE:set(sensor_data.getWOW_NoseLandingGear() == 1 and 1 or (sensor_data.getBarometricAltitude() < 50 and 1 or 0))
	parameters.GEAR_LEFT:set(sensor_data.getWOW_LeftMainLandingGear() == 1 and 1 or (sensor_data.getBarometricAltitude() < 50 and 1 or 0))
	parameters.GEAR_RIGHT:set(sensor_data.getWOW_RightMainLandingGear() == 1 and 1 or (sensor_data.getBarometricAltitude() < 50 and 1 or 0))

	-- Ensure display pages are active
	parameters.EICAS_ENG_PAGE:set(1.0)
	parameters.PFD_ADI_PAGE:set(1.0)
	if parameters.MFD_OPACITY:get() < 0.5 then
		parameters.MFD_OPACITY:set(1.0)
	end
	if parameters.UFD_OPACITY:get() < 0.5 then
		parameters.UFD_OPACITY:set(1.0)
	end

	parameters.FUELL        :set(sensor_data.getTotalFuelWeight() * 0.01 )
	parameters.FUEL         :set(sensor_data.getTotalFuelWeight() * 2.20462 )
	parameters.FUELT        :set(sensor_data.getTotalFuelWeight() * 2.20462 - 18078)
	parameters.TOTAL_FUEL_LBS:set(sensor_data.getTotalFuelWeight() * 2.20462)
